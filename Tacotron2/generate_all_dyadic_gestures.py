...
parser.add_argument('-fi', "--h5file_interlocutor", type=str, default="../val_interloctr_v1.h5")
...
for index in tqdm(range(len(h5.keys()))):
	...
	mfcc_interlocutor = torch.FloatTensor(np.zeros_like((h5_interlocutor[str(index)]["audio"]["mfcc"][:] - mfcc_mean) / mfcc_std))
	prosody_interlocutor = torch.FloatTensor(np.zeros_like((h5_interlocutor[str(index)]["audio"]["prosody"][:] - prosody_mean) / prosody_std))
